...
from app.models.clinic import (BranchTarget, Customer, DailyAdSpend, Employee,
                               Sale, SaleEmployee)
...
def get_risk_dashboard_data(
    ...
    if ticket_band:
        if ticket_band == "10k+":
            sales_q = sales_q.filter(Sale.sale_amount >= 10000)
        elif ticket_band == "5k-10k":
            sales_q = sales_q.filter(Sale.sale_amount >= 5000, Sale.sale_amount < 10000)
        elif ticket_band == "2.5k-5k":
            sales_q = sales_q.filter(Sale.sale_amount >= 2500, Sale.sale_amount < 5000)
        elif ticket_band == "1k-2.5k":
            sales_q = sales_q.filter(Sale.sale_amount >= 1000, Sale.sale_amount < 2500)
        elif ticket_band == "500-1k":
            sales_q = sales_q.filter(Sale.sale_amount >= 500, Sale.sale_amount < 1000)
        elif ticket_band == "0-500":
            sales_q = sales_q.filter(Sale.sale_amount >= 0, Sale.sale_amount < 500)

    # Optional staff filter
    if staff_name:
        sales_q = (
            sales_q.join(Sale.assigned_employees)
            .join(SaleEmployee.employee)
            .filter(Employee.full_name == staff_name)
        )
    ...
    # 2. Payment Mix
    payment_mix_raw = (
        db.query(Sale.payment_method, func.sum(Sale.sale_amount).label("amount"))
        .select_from(sales_q.subquery())
    ...
    payment_mix = {
        row.payment_method: float(row.amount or 0) for row in payment_mix_raw
    }
    ...
    prev_sales_q = db.query(Sale).filter(
        Sale.created_at >= prev_start_dt, Sale.created_at < prev_end_dt
    )
    prev_daily_raw = (
        prev_sales_q.with_entities(
            func.date(Sale.created_at).label("date"),
            func.sum(Sale.sale_amount).label("revenue"),
        )
        .group_by(func.date(Sale.created_at))
    ...
    visit_counts = (
        sales_q.with_entities(Sale.customer_id, func.count(Sale.id).label("visits"))
    ...
    for row in visit_counts:
        v = row.visits
        if v == 1:
            visit_frequency["1 visit"] += 1
        elif v == 2:
            visit_frequency["2 visits"] += 1
        elif v >= 3:
            visit_frequency["3+ visits"] += 1
    ...
    for (amount,) in bands_raw:
        a = float(amount or 0)
        if a <= 500:
            ticket_bands["0-500"] += 1
        elif a <= 1000:
            ticket_bands["500-1k"] += 1
        elif a <= 2500:
            ticket_bands["1k-2.5k"] += 1
        elif a <= 5000:
            ticket_bands["2.5k-5k"] += 1
        elif a <= 10000:
            ticket_bands["5k-10k"] += 1
        else:
            ticket_bands["10k+"] += 1
    ...
    staff_perf_raw = (
        sales_q.join(Sale.assigned_employees)
        .join(Employee, Employee.id == SaleEmployee.employee_id)
        .with_entities(
            Employee.full_name,
            func.sum(Sale.sale_amount).label("revenue"),
            func.count(Sale.id).label("count"),
    ...
    heatmap_raw = (
        ...
        .group_by("dow", "hour", Employee.full_name)
        .all()
    )
    ...
    vip_raw = (
        sales_q.join(Customer, Customer.id == Sale.customer_id)
        .with_entities(
            Customer.full_name,
            Customer.phone,
            func.sum(Sale.sale_amount).label("revenue"),
    ...
    today_stats = (
        snapshot_q.filter(Sale.created_at >= today_start)
        .with_entities(func.sum(Sale.sale_amount), func.count(Sale.id))
        .first()
    )
    yesterday_stats = (
        snapshot_q.filter(
            Sale.created_at >= yesterday_start, Sale.created_at < today_start
        )
        .with_entities(func.sum(Sale.sale_amount), func.count(Sale.id))
        .first()
    )
    last_week_stats = (
        snapshot_q.filter(
            ...
            Sale.created_at < last_week_start + timedelta(days=1),
        )
        .with_entities(func.sum(Sale.sale_amount))
        .first()
    )
    ...
    for i in range(1, 5):
        ...
        r = (
            snapshot_q.filter(Sale.created_at >= d_start, Sale.created_at < d_end)
            .with_entities(func.sum(Sale.sale_amount))
            .scalar()
        )
...
def get_marketing_analytics(days: int = 30, db: Session = Depends(get_db)):
    ...
    spend = (
        ...
        .filter(DailyAdSpend.created_at >= start_date)
        .group_by(DailyAdSpend.date)
        .all()
    )
