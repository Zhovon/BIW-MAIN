...
def punch_time(payload: PunchRequest, db: Session = Depends(get_db)):
    ...
    if not record or record.clock_out_time is not None:
        ...
        if not record:
            try:
                expected_start = local_now.replace(
...
def get_daily_roster(
    date: str, branch_id: Optional[str] = None, db: Session = Depends(get_db)
):
    ...
    for emp in employees:
        ...
        if emp_atts:
            ...
            total_deduction = sum(
                float(a.deduction_amount) for a in emp_atts if a.deduction_amount
            )
            ...
            roster.append(
                {
                    ...
                    "clock_in_time": (
                        first_att.clock_in_time.isoformat()
                        if first_att.clock_in_time
                        else None
                    ),
                    "clock_out_time": (
                        last_att.clock_out_time.isoformat()
                        if last_att.clock_out_time
                        else None
                    ),
