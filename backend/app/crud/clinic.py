...
from app.models.clinic import (AttendanceRecord, Branch, BranchTarget,
                               CostEntry, Customer, CustomerReview, Employee,
...
def list_service_assignments(db: Session) -> Sequence[ServiceAssignment]:
    return (
        db.query(ServiceAssignment).order_by(ServiceAssignment.created_at.desc()).all()
    )
...
def _attach_totals(customers, db: Session):
    ...
    agg_map = {
        row.customer_id: {"visits": row.total_visits, "spent": row.total_spent}
        for row in aggs
    }
...
def get_sale_employee_ids(db: Session, sale_id: str) -> list[str]:
    rows = (
        db.query(SaleEmployee.employee_id).filter(SaleEmployee.sale_id == sale_id).all()
    )
...
def list_branch_targets(
    db: Session, branch_id: Optional[str] = None
) -> Sequence[BranchTarget]:
...
def list_attendance_records(
    db: Session, employee_id: Optional[str] = None
) -> Sequence[AttendanceRecord]:
    query = db.query(AttendanceRecord)
    if employee_id:
        query = query.filter(AttendanceRecord.employee_id == employee_id)
    return query.order_by(
        AttendanceRecord.date.desc(), AttendanceRecord.created_at.desc()
    ).all()
